...
%% Efficient Implementation (2025/04/14)
problem.cost  = @(x) sum(pagenorm(Fopt - pagemtimes(reshape(x, Nt, NRF), FBB), 'fro').^2, "all");
problem.egrad = @(x) reshape(-2 * sum(pagemtimes((Fopt - pagemtimes(reshape(x, Nt, NRF), FBB)), 'none', FBB, 'ctranspose'), 3), [], 1);
%%
