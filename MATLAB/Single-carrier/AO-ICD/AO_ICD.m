function [FRF, FBB, WRF, WBB, iterF, iterW] = AO_ICD(H, Ns, NRF, SNR)
    % Hybrid beamforming design based on Algorithm 1 and Algorithm 2 (2016)
    ...
    % Analog Precoder 
    F1  = H' * H;
    % FRF = Algorithm_1(F1, NRF, SNR / Ns / Nt);
    % FRF = Algorithm_1(F1, NRF, SNR / NRF / Nt);
    [FRF, iterF] = Algorithm_1(F1, NRF, gamma2 / sigma2);
    ...
    % WRF = Algorithm_1(F2, NRF, SNR / Ns / Nr);
    [WRF, iterW] = Algorithm_1(F2, NRF, 1 / Nr / sigma2);
...
function [VRF, iter] = Algorithm_1(F, Nrf, a)
    [N, ~] = size(F);
    VRF = ones(N, Nrf);
    delta = 1;
    VRF0 = VRF;
    ...
    while delta > 1e-3
        ...
        % delta = mean(abs(angle(exp(1i * (angle(VRF) - angle(VRF0))))), "all");
        % delta = mean(abs(VRF - VRF0), "all");        
